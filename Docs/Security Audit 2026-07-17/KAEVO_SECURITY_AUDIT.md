...
| KSEC-011A | High | Owner enrollment | Owner bootstrap manually serialized DynamoDB values and then passed them through the boto3 resource client, which serialized them again; the five-record transaction failed closed with a key type mismatch. | **Remediated in canonical source; operationally open**: native Python records now pass through exactly one boto3 serialization layer. Actual wire-shape and DynamoDB Local atomicity/replay/concurrency/conflict regressions pass. Staging Lambda redeployment and real synthetic enrollment proof remain required. See `KAEVO_COGNITO_AUTHORITATIVE_CLAIMS.md`. |
...
| KSEC-012A | High | Home connector lifecycle | Connector pairing lacked immutable server binding, monotonic key versions, secure rotation/recovery, atomic audit evidence, and explicit destructive unpair. | **Operationally closed in isolated staging**: candidate `7933a14` passed all 26 lifecycle checks with zero synthetic records after cleanup; production migration remains separately gated. |

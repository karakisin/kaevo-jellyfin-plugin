...
public sealed class FirebaseRelayDemandSupervisorTests
{
    ...
    public async Task AcceptedTicketStartupRaceGetsOnlyOneShortRetryPerChannel()
    {
        ...
        {
            var count = Interlocked.Increment(ref attempts[index]);
            return Task.FromException(count <= 3
                ? new Exception("ticket-not-ready") : new FirebaseRelayStartupRaceException());
        }, _ => { }, stop.Token, clock, clock.Delay);
        demands.Writer.TryWrite(Demand(clock));
        await Until(() => attempts.All(n => n == 1) && clock.Timers == 4);
        foreach (var wait in new[] { 1, 2, 4 })
        {
            var before = attempts[0]; clock.Advance(wait);
            await Until(() => attempts.All(n => n == before + 1) && clock.Timers == 4);
        }
        // Previous ticket failures would have imposed eight more seconds.
        clock.Advance(1); await Until(() => attempts.All(n => n == 5) && clock.Timers == 4);
        // A persistently unavailable listener must return to bounded backoff.
        clock.Advance(1); await Task.Yield(); Assert.All(attempts, n => Assert.Equal(5, n));
        clock.Advance(1); await Until(() => attempts.All(n => n == 6) && clock.Timers == 4);
        stop.Cancel();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => run.WaitAsync(TimeSpan.FromSeconds(5)));
        Assert.Equal(0, clock.Timers);
    }

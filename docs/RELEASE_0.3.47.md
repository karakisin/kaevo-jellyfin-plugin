...
File discovery, reads and observation lifetime are bounded: at most 64 candidate names per search, eight searches, 512 KiB total reads, and the existing maximum 30-second origin operation. Unrelated, ambiguous, old, replaced, truncated or symbolic-link logs are rejected. Errors disable log observation without failing playback. Sampling and log flush timing mean these are observations, not exact CPU/GPU execution timestamps.

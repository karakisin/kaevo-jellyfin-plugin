...
- After source publication, a fresh `2026-09-03T20:39:04Z` readiness check reconfirmed green Lightsail `ACTIVE`, `nano`, scale one; `https-only` origin; direct health 200; direct protected access 403; edge health 200; invalid edge grant 401; and production still different from the green URL with the legacy distribution retained. Build 161 was relaunched on the isolated iPhone SE. The development table contained one active protocol-2 connector with a current TTL, and no remote request record was created during the first eight seconds after launch.

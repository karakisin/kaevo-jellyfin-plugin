"""Apple PKI roots bundled for App Store signed-data verification.

These public DER certificates were downloaded from Apple's official PKI root
certificate section on 2026-09-01. Keep the fingerprints beside the payloads so
certificate refreshes are reviewable and cannot silently replace trust roots.
"""

APPLE_ROOT_CERTIFICATES_BASE64 = (
    # Apple Root CA
    # SHA-256 B0:B1:73:0E:CB:C7:FF:45:05:14:2C:49:F1:29:5E:6E:
    #        DA:6B:CA:ED:7E:2C:68:C5:BE:91:B5:A1:10:01:F0:24
    """
    
    XUKqK1drk/NAJBzewdXUh
    """,
    # Apple Root CA - G2
    # SHA-256 C2:B9:B0:42:DD:57:83:0E:7D:11:7D:AC:55:AC:8A:E1:
    #        94:07:D3:8E:41:D8:8F:32:15:BC:3A:89:04:44:A0:50
    """
    
    """,
    # Apple Root CA - G3
    # SHA-256 63:34:3A:BF:B8:9A:6A:03:EB:B5:7E:9B:3F:5F:A7:BE:
    #        7C:4F:5C:75:6F:30:17:B3:A8:C4:88:C3:65:3E:91:79
    """
    
    """,
)

APPLE_ROOT_CERTIFICATE_SHA256 = (
    "b0b1730ecbc7ff4505142c49f1295e6eda6bcaed7e2c68c5be91b5a11001f024",
    "c2b9b042dd57830e7d117dac55ac8ae19407d38e41d88f3215bc3a890444a050",
    "63343abfb89a6a03ebb57e9b3f5fa7be7c4f5c756f3017b3a8c488c3653e9179",
)
